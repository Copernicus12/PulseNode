...
<div class="space-y-5">
    <section class="overflow-hidden rounded-[2rem] border border-border/50 bg-card">
            ...
            <div class="grid gap-6 xl:grid-cols-[1.15fr_0.85fr] xl:items-end">
                <div>
                    ...
                    <div class="mt-7 grid gap-3 sm:grid-cols-4">
                        <div class="rounded-3xl border border-sky-500/25 bg-sky-500/8 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
                            <p class="text-[10px] uppercase tracking-[0.26em] text-muted-foreground">Reserve score</p>
                        ...
                        <div class="rounded-3xl border border-amber-500/25 bg-amber-500/8 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
                            <p class="text-[10px] uppercase tracking-[0.26em] text-muted-foreground">Standby waste</p>
                        ...
                        <div class="rounded-3xl border border-emerald-500/25 bg-emerald-500/8 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
                            <p class="text-[10px] uppercase tracking-[0.26em] text-muted-foreground">Stability</p>
                        ...
                        <div class="rounded-3xl border border-violet-500/25 bg-violet-500/8 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
                            <p class="text-[10px] uppercase tracking-[0.26em] text-muted-foreground">Live draw</p>
                ...
                <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
                    <div class="rounded-3xl border border-border/40 bg-background p-5">
                        <div class="flex items-center justify-between gap-4">
                            <div>
                                <p class="text-[10px] uppercase tracking-[0.26em] text-muted-foreground">Main gauge</p>
                                <p class="mt-2 text-sm font-semibold">Energy reserve ring</p>
                                <p class="mt-1 text-xs text-muted-foreground">Fast visual estimate of overall strip efficiency.</p>
                            </div>
                            <div class="relative flex h-28 w-28 items-center justify-center rounded-full border border-border/40 bg-card">
                                <div class="absolute inset-2 rounded-full border-[10px] border-muted/70"></div>
                                <div class="absolute inset-2 rounded-full border-[10px] border-transparent {{ $score >= 76 ? 'border-t-emerald-400 border-r-emerald-400' : ($score >= 56 ? 'border-t-amber-400 border-r-amber-400' : 'border-t-red-400 border-r-red-400') }}" style="transform: rotate({{ max(15, min(330, $score * 3.3)) }}deg);"></div>
                    ...
                    <div class="rounded-3xl border border-border/40 bg-zinc-950 p-5 text-zinc-100">
                        <div class="flex items-center justify-between gap-3">
                            <div>
                                <p class="text-[10px] uppercase tracking-[0.26em] text-zinc-400">Operator note</p>
                                <p class="mt-2 text-sm font-semibold">Interpretation</p>
                            </div>
                            <span class="inline-flex rounded-full border border-zinc-700 px-2.5 py-1 text-[11px] text-zinc-300">Live</span>
                        </div>
                        <p class="mt-3 text-sm text-zinc-300">High score means the strip is spending most of its power on useful, stable loads. Low score usually means standby drift, volatile appliances, or inefficient always-on devices.</p>
                        <div class="mt-4 grid grid-cols-2 gap-3 border-t border-zinc-800 pt-4 text-xs text-zinc-400">
                            <p>Last sync {{ $lastSeen }}</p>
                            <p class="text-right">System {{ ucfirst($systemStatus) }}</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <section class="grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
        <div class="flex flex-col gap-5">
            <div class="rounded-3xl border border-border/40 bg-card p-7">
                ...
                <div class="mt-5 space-y-3">
                    @foreach($battery['priority_actions'] as $action)
                        <div class="rounded-3xl border border-border/30 bg-background p-4">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <p class="text-[10px] uppercase tracking-[0.24em] text-muted-foreground">Action item</p>
                                    <p class="mt-2 text-sm font-semibold">{{ $action['title'] }}</p>
        ...
        <div class="rounded-3xl border border-border/40 bg-card p-7">
            ...
            <div class="mt-6 grid gap-5 lg:grid-cols-3">
                @foreach($battery['socket_insights'] as $socket)
                    <article class="overflow-hidden rounded-[2rem] border border-border/30 bg-background shadow-[inset_0_1px_0_rgba(255,255,255,0.02)]">
                        <div class="p-6">
                            <div class="flex items-start justify-between gap-3">
                                <div>
                            ...
                            <div class="mt-5 grid grid-cols-2 gap-3 text-sm">
                                <div class="rounded-2xl border border-emerald-500/25 bg-emerald-500/8 p-4">
                                    <p class="text-[10px] uppercase tracking-[0.22em] text-muted-foreground">Efficiency</p>
                                    <p class="mt-1 text-lg font-bold tabular-nums">{{ $socket['efficiency_score'] }}</p>
                                </div>
                                <div class="rounded-2xl border border-sky-500/25 bg-sky-500/8 p-4">
                                    <p class="text-[10px] uppercase tracking-[0.22em] text-muted-foreground">Stability</p>
                                    <p class="mt-1 text-lg font-bold tabular-nums">{{ $socket['stability_score'] }}</p>
                                </div>
                            </div>

                            <div class="mt-5 rounded-3xl border border-border/25 bg-card/50 p-4">
                                <div class="mb-4 flex items-center justify-between">
                                    <p class="text-[10px] uppercase tracking-[0.26em] text-muted-foreground">Diagnostics</p>
                                    <span class="text-[11px] text-muted-foreground">Recent window</span>
                                </div>
                                <div class="space-y-4">
                                <div>
                            ...
                            <div class="mt-5 grid grid-cols-2 gap-3 border-t border-border/20 pt-4 text-sm">
                                <div class="rounded-2xl border border-border/20 bg-card/40 p-3">
                                    <p class="text-[10px] uppercase tracking-[0.22em] text-muted-foreground">Avg / Peak</p>
                                    <p class="mt-1 font-semibold tabular-nums">{{ number_format($socket['avg_power_w'], 1) }} / {{ number_format($socket['peak_power_w'], 1) }} W</p>
                                </div>
                                <div class="rounded-2xl border border-border/20 bg-card/40 p-3">
                                    <p class="text-[10px] uppercase tracking-[0.22em] text-muted-foreground">Active / Standby</p>
