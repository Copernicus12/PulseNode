...
const toasterOptions = computed(() => ({
    ...
    classes: {
        ...
        actionButton: '!h-7 !rounded-lg !border !border-red-400/45 !bg-red-500/18 !px-2.5 !text-[11px] !font-medium !text-red-50 hover:!bg-red-500/28',
...
function handleNotificationEvent(event: Event): void {
    ...
    showNotification({
        message: customEvent.detail?.message || nextGuard.message || 'Unable to turn relay on right now.',
...
onBeforeUnmount(() => {
    ...
    window.removeEventListener('pulsenode:latest', updateOffset as EventListener)
